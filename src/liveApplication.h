...
namespace liveApplication {
    ...
    public:
        std::shared_ptr<CommonColor> commonColor;
        std::shared_ptr<MainPageSummaryInterface> interfaceVariable;
    ...
    public:
        ...
        Element Render() noexcept final {
            if (_depth.pushedReturn && _depth.pageCode == 0b0001) {
                if (!interfaceVariable->summaryIsSelected) { return renderMainPage(); }

                if ((interfaceVariable->selectedSummary & 0b0001) && thereIsADailySummary) {
                    auto dailySummary = renderDailySummaryPage();
                    return std::move(dailySummary);
                }
                if ((interfaceVariable->selectedSummary & 0b0010) && thereIsAWeeklySummary) {
                    auto weeklySummary = renderWeeklySummaryPage();
                    return std::move(weeklySummary);
                }
                if ((interfaceVariable->selectedSummary & 0b0100) && thereIsAMonthlySummary) {
                    auto monthlySummary = renderMonthlySummaryPage();
                    return std::move(monthlySummary);
                }
                if ((interfaceVariable->selectedSummary & 0b1000) && thereIsAYearlySummary) {
        ...
        bool OnEvent(Event event) noexcept override {
            if (event == Event::Escape) {
                ...
                interfaceVariable->summaryIsSelected = false;
                return Component::OnEvent(event);
            }
    ...
    private:
        ...
        std::shared_ptr<MainPageSummaryInterface> _interfaceVariable = std::make_shared<MainPageSummaryInterface>();
