...
void backend::FileSystem::_fillFileNameList(
    ...
    _fillGivenListWithAbsoluteFilePath(list, localPath);
}
...
void backend::FileSystem::_createFolder(const std::string& folderName) noexcept {
    ...
    if (folderName.empty() && mkdir(_path)) { return; }
...
bool backend::FileSystem::_isEmpty(std::ifstream& pFile) noexcept {
    return pFile.peek() == std::ifstream::traits_type::eof();
}

void backend::FileSystem::_applyHomepathNameTakenFromEnvironment() noexcept {
    std::string tempUserName = getenv("HOMEPATH");
    auto stringSize = tempUserName.size();

    for (int i = 0; i < stringSize; ++i) {
        _homepathName[i] = tempUserName.at(i);
    }
}

void backend::FileSystem::_fillGivenListWithFileNames(
    ...
    if ( hFind != INVALID_HANDLE_VALUE ) {
        do {
            tempString = data.cFileName;

            if (tempString[0] == '.' || tempString[1] == '.') { continue; }
            elementList.emplace_back(data.cFileName);
...
void backend::FileSystem::_removeFiles(std::string& folderPath) const noexcept {

    std::vector<std::string> fileList{};

    _fillGivenListWithFileNames(fileList, folderPath);
...
[[nodiscard]] bool backend::FileSystem::_removeFolder(const std::string& folderPath) const noexcept {
    return RemoveDirectoryA(folderPath.c_str());
}

/// removes given folder whether empty or not.
bool backend::FileSystem::_remove_all(std::string folderAbsolutePath) noexcept {
    _removeFiles(folderAbsolutePath);

    std::vector<std::string> folderPathList{};
    _fillGivenListWithAbsoluteFilePath(folderPathList, folderAbsolutePath);
    if (!folderPathList.empty()) {
        for (auto& folderPath : folderPathList) {
            _remove_all(folderPath);
        }
    }

    if (!_removeFolder(folderAbsolutePath)) { return false; };

    return true;
}

void backend::FileSystem::_fillGivenListWithAbsoluteFilePath(
        std::vector<std::string>& elementContainer,
        std::string& folderPath
        ) const noexcept {

    std::vector<std::string> fileNameList{};
    _fillGivenListWithFileNames(fileNameList, folderPath);

    std::string absolutePath{};
    for (auto& fileName : fileNameList) {
        absolutePath = folderPath;
        absolutePath.append("\\" + fileName);
        elementContainer.emplace_back(absolutePath);
        absolutePath = {};
    }
}
...
void backend::FileSystem::setup() {
    ...
    _cleanCharString(_homepathName, UNAME_MAX);
    _applyHomepathNameTakenFromEnvironment();

    _cleanCharString(_tempPath, PATH_MAX);
    _cleanCharString(_path, PATH_MAX);

    // Main folder setup
    _appendCharString(_path, PATH_MAX, _homepathName, MAX_PATH);
...
void backend::FileSystem::saveFile(
    ...
    if (readOnlyFile.is_open()) {
        ...
        if (_isEmpty(readOnlyFile) || clearAndWrite) {
            readOnlyFile.close();
