...
namespace backend {
    ...
    private:
        ...
        bool _isEmpty(std::ifstream& pFile) noexcept;

        void _applyHomepathNameTakenFromEnvironment() noexcept;

        void _fillGivenListWithFileNames(
                std::vector<std::string>& elementList,
                std::string& folderPath
                ) const noexcept;

        void _removeFiles(std::string& folderPath) const noexcept;

        [[nodiscard]] bool _removeFolder(const std::string& folderPath) const noexcept;

        /// @brief removes given folder whether empty or not.
        bool _remove_all(std::string folderAbsolutePath) noexcept;

        void _fillGivenListWithAbsoluteFilePath(
